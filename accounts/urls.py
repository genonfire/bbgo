...
urlpatterns = [
    url(
        r'^login/$',
        auth_views.LoginView.as_view(template_name='accounts/login.html', authentication_form=forms.LoginForm),
        name='login'
    ),
    url(
        r'^logout/$',
    ...
    url(
        r'^dashboard_user_search/(?P<search_word>.*)/(?P<condition>.*)/(?P<page>\d+)/$',
        views.dashboard_user,
